...
/***
 * This class of tests uses only the web layer.
 * IT DOESN'T WORK, because the spring context isn't loaded, so spring can't inject the repository into the controller.
 * IT'S JUST A TEMPLATE, something to remember how to test only the application's web layer.
 */
@WebMvcTest
@ActiveProfiles("test")
public class ContactControllerTests_webMvcTest {

    @Autowired
    private MockMvc mockMvc;

    //it tests a get request comparing the request's result to an object already saved in db during the initialization
    //@Test THIS ANNOTATION IS COMMENTED BECAUSE THIS TEST ALWAYS FAIL (AND IT IS SUPPOSED TO)
